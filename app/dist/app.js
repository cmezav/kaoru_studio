...
const studios = [
    ...
    { id: 'image', name: 'Image Studio', short: 'Imagen', subtitle: 'Ajustes, filtros y exportación', src: './legacy/image-launcher/index.html?embed=1', accent: 'blue', icon: h(Icon, { name: 'image' }), hotkey: 'Alt+3' },
...
class App extends React.Component {
    ...
    onKey(e) {
        if (!e.altKey || e.ctrlKey || e.metaKey || e.shiftKey)
            return;
        const map = { '1': 'silhouette', '2': 'text', '3': 'image', '4': 'gallery' };
        if (!map[e.key])
            return;
        e.preventDefault();
