(function () {
  ...
  function studioUrl(studio) {
    ...
    const folders = { text: 'text-studio', image: 'image-studio', light: 'light-lab', '3d': '3d-lighting', gallery: 'gallery' };
  ...
  function installNavigationShortcuts() {
    document.addEventListener('keydown', (event) => {
      ...
      const target = event.key === '1' ? 'silhouette' : event.key === '2' ? 'text' : event.key === '3' ? 'image' : event.key === '4' ? 'light' : event.key === '5' ? '3d' : event.key === '6' ? 'gallery' : null;
      if (!target) return;
